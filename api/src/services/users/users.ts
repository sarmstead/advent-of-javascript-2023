...
export const User: UserRelationResolvers = {
  ...
  thankYouUserFrom: (_obj, { root }) => {
    return db.user.findUnique({ where: { id: root?.id } }).thankYouUserFrom()
  },
  thankYouUserTo: (_obj, { root }) => {
    return db.user.findUnique({ where: { id: root?.id } }).thankYouUserTo()
  },
}
